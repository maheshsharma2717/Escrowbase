...
namespace SR.EscrowBaseWeb.Authorization.Accounts
{
    public class AccountAppService : EscrowBaseWebAppServiceBase, IAccountAppService
    {
        ...
        public async Task<ResetPasswordOutput> ResetPassword(ResetPasswordInput input)
        {
            ...
            if(input.IsOTP == true)
            {
                if(user.EmailConfirmationCode != input.OTP)
                {
                    throw new UserFriendlyException(L("InvalidOTP"), L("InvalidOTP"));
                }
            }
        ...
        public async Task<ResetPasswordOutput> ResetPasswordByAdmin(ResetPasswordByAdmin input)
        {
            var user = await UserManager.GetUserByIdAsync(input.UserId);
            if (user == null  )
            {
                throw new UserFriendlyException(L("InvalidUser"), L("InvalidUser"));
            }
