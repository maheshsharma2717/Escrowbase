...
using SR.EscrowBaseWeb.Authorization.Permissions;
...
using SR.EscrowBaseWeb.Authorization.Users.Exporting;
...
namespace SR.EscrowBaseWeb.Authorization.Users
{
    ...
    public class UserAppService : EscrowBaseWebAppServiceBase, IUserAppService
    {
        ///<Summary>
        /// Get aand Set AppURL
        ///</Summary>
        ...
        ///<Summary>
        /// Collect all data for user app service
        ///</Summary>
        ...
        /// Create user
        ///</Summary>
        public virtual async Task CreateUserAsync(CreateOrUpdateUserInput input)
        {
            try
            {
                var check =  _userRepository.GetAll().Where(x => x.EmailAddress == input.User.EmailAddress).FirstOrDefault();
                if(check != null)
                {
                 //   CreateOrUpdateUserInput item = new CreateOrUpdateUserInput();
                  //  item.User.Id = check.Id;
                 //  input.User.Id = check.Id;
                  //  return;
                }
                ...
                var g = CurrentUnitOfWork.SaveChangesAsync(); //To get new user's Id.
                g.Wait();
                //Notifications
        ...
        ///<Summary>
        /// Get user filtered query
        ///</Summary>
        public IQueryable<User> GetUsersFilteredQuery(IGetUsersInput input)
        {
        ...
        /// Create new user
        ///</Summary>
        public void CreateNewUser(CreateOrUpdateUserInput input)
        {
            if (AbpSession.TenantId.HasValue)
            {
                _userPolicy.CheckMaxUserCountAsync(AbpSession.GetTenantId());
            }
            ...
            if (input.SetRandomPassword)
            {
                var randomPassword = _userManager.CreateRandomPassword();
                string newpas = Convert.ToString(randomPassword);
                user.Password = _passwordHasher.HashPassword(user, newpas);
                input.User.Password = newpas;
            }
            else if (!input.User.Password.IsNullOrEmpty())
            {
                UserManager.InitializeOptionsAsync(AbpSession.TenantId);
                foreach (var validator in _passwordValidators)
                {
                    //CheckErrors(validator.ValidateAsync(UserManager, user, input.User.Password));
                }
            ...
            foreach (var roleName in input.AssignedRoleNames)
            {
                var role =  _roleManager.GetRoleByNameAsync(roleName);
                user.Roles.Add(new UserRole(AbpSession.TenantId, user.Id, role.Id));
            }

            //CheckErrors(UserManager.CreateAsync(user));
            CurrentUnitOfWork.SaveChangesAsync(); //To get new user's Id.

            //Notifications
            _notificationSubscriptionManager.SubscribeToAllAvailableNotificationsAsync(user.ToUserIdentifier());
            _appNotifier.WelcomeToTheApplicationAsync(user);

            //Organization Units
            UserManager.SetOrganizationUnitsAsync(user, input.OrganizationUnits.ToArray());

            //Send activation email
            if (input.SendActivationEmail)
            {
                user.SetNewEmailConfirmationCode();
                _userEmailer.SendEmailActivationLinkAsync(
                    user,
                    AppUrlService.CreateEmailActivationUrlFormat(AbpSession.TenantId),
                    input.User.Password
                );
