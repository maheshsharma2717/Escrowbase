...
using SR.EscrowBaseWeb.Web.Authentication.JwtBearer;
using SR.EscrowBaseWeb.Web.Authentication.TwoFactor;
using SR.EscrowBaseWeb.Web.Chat.SignalR;
using SR.EscrowBaseWeb.Web.Configuration;
using SR.EscrowBaseWeb.Web.DashboardCustomization;

namespace SR.EscrowBaseWeb.Web
{
    [DependsOn(
        typeof(EscrowBaseWebApplicationModule),
        typeof(EscrowBaseWebEntityFrameworkCoreModule),
    ...
    public class EscrowBaseWebWebCoreModule : AbpModule
    {
        private readonly IWebHostEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public EscrowBaseWebWebCoreModule(IWebHostEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(EscrowBaseWebApplicationModule).GetAssembly()
                );

            Configuration.Caching.Configure(TwoFactorCodeCacheItem.CacheName, cache =>
            {
                cache.DefaultAbsoluteExpireTime = Convert.ToDateTime(TimeSpan.FromMinutes(2));
            });
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(EscrowBaseWebWebCoreModule).GetAssembly());
        }
        ...
        public override void PostInitialize()
        {
            ...
            IocManager.Resolve<ApplicationPartManager>()
                .AddApplicationPartsIfNotAddedBefore(typeof(EscrowBaseWebWebCoreModule).Assembly);
        }
