...
namespace SR.EscrowBaseWeb.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : EscrowBaseWebServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("SR.EscrowBaseWeb", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
