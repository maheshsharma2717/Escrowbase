...
import { EscrowBaseWebCommonModule } from '@shared/common/common.module';
...
function initializeLocalForage() {
    localForage.config({
        ...
        description: 'Cached data for EscrowBaseWeb'
    });
}
...
function initializeTenantResources(injector: Injector) {
    ...
    if (metaImage) {
        ...
        if (!appSessionService.tenant || !appSessionService.tenant.logoId) {
            ...
            metaImage.setAttribute('content', window.location.origin + '/assets/common/images/Escrow-logo.png');
        } else {
