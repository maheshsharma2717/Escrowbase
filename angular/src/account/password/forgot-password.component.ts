...
this.getQuestion();
    ...
    Swal.fire({
        icon: 'error', // You can choose between 'success', 'error', 'warning', 'info', 'question'
        title: 'Invalid Email Please enter the correct email',
        toast: true,
        position: 'top-end', // You can change the position to 'top-end', 'bottom-end', etc.
        showConfirmButton: false, // Removes the confirmation button for the toast
        timer: 3000, // Duration for the toast to appear
        timerProgressBar: true, // Shows the progress bar for the timer
      });
}
        });
    }
    ...
    getQuestion() {
        ...
        this._userAnswersServiceProxy.getAll(
            ...
            // Populate the primary list of questions
            data.forEach(element => {
                this.quesData.push(element.userAnswer.question);
            });
    
            // quesData1 and quesData2 will be dynamically filtered in the template
            this.quesData1 = [...this.quesData]; // Make a copy for reactive purposes
            this.quesData2 = [...this.quesData]; // Make a copy for reactive purposes
        });
    }
    ...
    verifyAnswer(){
        ...
        this._userAnswersServiceProxy.getAll(
         ...
         if(phone == null){
            ...
            this.http.get(url+"/Home/forgotPassword?uid="+this.userid+"&un="+this.model.emailAddress+"&phoneno="+phone+"&code=&status=Code")
                .subscribe((res)=> {
            this.showotp=true;
    ...
    verify(){
        ...
        let url = AppConsts.remoteServiceBaseUrl;
        this.http.get(url+"/Home/forgotPassword?uid="+this.userid+"&un="+this.model.emailAddress+"&phoneno="+phone+"&code="+this.otp+"&status=Reset")
      .subscribe((res:any)=> {
          if(res.result.statusCode ==200){
            this.message.success(this.l('PasswordResetMailSentMessage'), this.l('MailSent')).then(() => {
                this._router.navigate(['account/login']);});
        }else{
            this.message.warn( res.result.message, this.l('')).then(() => {
                ;});
        }
