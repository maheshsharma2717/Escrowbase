...
export class NotificationsComponent extends AppComponentBase {
    ...
    loading = false;
    dissableReadButton:boolean = false;
    public dateRangeCustome: Date[] = [];
    constructor(
        injector: Injector,
    ...
    setAsRead(record: any): void {
        this.dissableReadButton = true
        this.setNotificationAsRead(record, () => {
            this.reloadPage();
            this.dissableReadButton =false;
        });
    }
    ...
    getNotifications(event?: LazyLoadEvent): void {
        ...
        let startDateTime = this._dateTimeService.getStartOfDayForDate(this.dateRange[0]);
        let endDateTime = this._dateTimeService.getEndOfDayForDate(this.dateRange[1]);

        if ((startDateTime as any)._isAMomentObject === true) {
            startDateTime = DateTime.fromMillis(startDateTime.valueOf());
            endDateTime = DateTime.fromMillis(endDateTime.valueOf());
        }
        ...
        this._notificationService.getUserNotifications(
            ...
            endDateTime,
            //start1,
            //this._dateTimeService.getEndOfDayForDate(this.dateRange[1].toISODate()),
            ...
            (isConfirmed) => {
                if (isConfirmed) {
                    this._notificationService.deleteAllUserNotifications(
                        ...
                        this._dateTimeService.getEndOfDayForDate(this.dateRange[1]).endOf('day')).subscribe(() => {
                            this.reloadPage();
                            this.notify.success(this.l('SuccessfullyDeleted'));
                        });
                }
