...
export class DateTimeService {
    ...
    fromNowCustom(date: DateTime | Date | string): string {
        if (typeof date === 'string') {
            // Assuming Luxon is used
            date = DateTime.fromISO(date); // Convert ISO string to DateTime
        }
    
        if (date instanceof Date) {
            return this.fromNow(DateTime.fromJSDate(date)); // Convert JS Date to DateTime
        }
    
        return date.toRelative(); // Luxon's toRelative for human-readable "time ago"
    }
}
